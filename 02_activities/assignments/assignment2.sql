...
SELECT product_name || ', ' || product_size|| ' (' || product_qty_type || ')' FROM product
...
-- Step 2: Get the relevant vendor and product information
VendorProductInfo AS (
    SELECT 
        v.vendor_name,
        p.product_name,
        vi.original_price
...
-- Step 3: Use a subquery with CROSS JOIN to calculate total sales
SELECT 
    vpi.vendor_name,
    vpi.product_name,
    vpi.original_price * 5 * cc.customer_count AS total_sales
FROM 
    VendorProductInfo vpi
CROSS JOIN 
    (SELECT customer_count FROM CustomerCount) cc
GROUP BY 
    vpi.vendor_name, 
    vpi.product_name, 
    vpi.original_price, 
...
--Step 1: Add current_quantity column to product_units
ALTER TABLE product_units
ADD current_quantity INT;
...
WITH LastQuantity AS (
    ...
    LEFT JOIN (
        ...
        FROM 
            vendor_inventory
    ) vi ON pu.product_id = vi.product_id AND vi.rn = 1
)

-- Step 3: Update the current_quantity in the product_units table
...
SET current_quantity = (
    ...
    FROM 
        LastQuantity 
    WHERE 
        LastQuantity.product_id = product_units.product_id
);